...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 w-full py-2 px-4 border-b border-primary-900/20 bg-black/90 backdrop-blur-sm shadow-lg z-50">
      <div className="max-w-[1400px] mx-auto flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          {userRole === 'admin' && (
            <Link href="/admin" className="px-3 py-1.5 bg-yellow-600 text-white rounded-md hover:bg-yellow-700 transition-all duration-200 font-semibold text-xs tracking-wide">
              ADMIN PANEL
            </Link>
          )}
          {userRole === 'booster' && (
            <Link href="/hub" className="px-3 py-1.5 bg-primary-600 text-white rounded-md hover:bg-primary-700 transition-all duration-200 font-semibold text-xs tracking-wide">
              BOOSTER HUB
            </Link>
          )}
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="relative group">
            ...
            <div className="absolute left-0 top-full mt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
              <div className="bg-gray-900 border border-primary-700/50 rounded-lg shadow-2xl p-3 min-w-[200px]">
                {/* Game Item */}
                <Link
                  href="/games/black-ops-7"
                  className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-primary-700/30 transition-colors duration-200 group/item"
                >
          ...
          {user ? (
            <div className="relative account-menu-container">
              <button
                ...
                className="p-1.5 hover:bg-gray-800/50 rounded-lg transition-all duration-200 group"
              >
              ...
              {showAccountMenu && (
                <div className="absolute right-0 top-full mt-2 w-48 bg-gray-900 border border-primary-700/50 rounded-lg shadow-2xl p-2 z-50">
                  <div className="px-2 py-1.5 border-b border-gray-700 mb-1.5">
                    <p className="text-xs text-gray-400">Signed in as</p>
                    <p className="text-sm text-white font-medium truncate">{userName}</p>
                  </div>
                  <Link
                    href="/account"
                    className="block px-2 py-1.5 text-sm text-gray-300 hover:bg-primary-700/30 hover:text-white rounded-md transition-colors duration-200"
                  ...
                  <button
                    ...
                    className="w-full text-left px-2 py-1.5 text-sm text-gray-300 hover:bg-red-900/30 hover:text-red-400 rounded-md transition-colors duration-200"
                  >
                    Logout
                  </button>
                </div>
            ...
            <Link href="/login" className="px-4 py-2 bg-gray-800/50 text-white border border-primary-600/50 rounded-md hover:bg-primary-600/20 hover:border-primary-600 transition-all duration-200 font-semibold text-sm tracking-wide">
              LOGIN
            </Link>
          )}

          {/* Cart Icon */}
          <Link href="/cart" className="relative p-1.5 hover:bg-gray-800/50 rounded-lg transition-all duration-200 group">
            <svg className="w-5 h-5 text-gray-300 group-hover:text-primary-400 transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
