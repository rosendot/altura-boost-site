...
export async function POST(request: Request) {
  try {
    ...
    if (!rateLimitResult.allowed) {
      await logAuthFailure(user.id, 'identity_verification', 'Rate limit exceeded', request);
    ...
    if (userData.role !== 'booster' || userData.booster_approval_status !== 'approved') {
      ...
      return NextResponse.json({ error: 'Only approved boosters can verify identity' }, { status: 403 });
    }
    ...
    try {
      ...
      if (updateError) {
        console.error('[IdentityVerify] Database update failed');
      ...
      return NextResponse.json({ url: verificationSession.url }, { headers: getRateLimitHeaders(rateLimitResult) });
    } catch (stripeError: any) {
      console.error('[IdentityVerify] Stripe error:', stripeError?.type, stripeError?.code);
      throw stripeError;
    }
  } catch (error: any) {
    console.error('[IdentityVerify] Error:', error?.type || 'unknown');
